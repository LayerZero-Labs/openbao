...
**It signs a digest you supply, and never hashes anything itself.** Unlike
...
`prehashed=true`, and `hash_algorithm` is ignored.
...
bytes. Every other Transit signing key type uses random nonces.

**Signatures are low-S on output, but high-S is accepted on input.** Produced
signatures are BIP-62 canonical, which Bitcoin and Ethereum require.
Verification accepts either form, so `transit/verify` may report `valid: true`
for a signature that a chain's consensus rules would reject.
...
signing under an address the chain no longer accepts. `auto_rotate_period` is
therefore rejected both at creation and via `keys/config`. Manual rotation is
...
**Not supported for this key type:** key import (BYOK), key agreement
(`datakey`/ECDH), CSR generation and certificate binding, and encryption. Go's
...
**This curve is not FIPS-approved.** Nothing in OpenBao breaks when it is used,
but offering it undercuts any FIPS claim a deployment makes.
