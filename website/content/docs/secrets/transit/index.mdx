...
- `ecdsa-secp256k1`: ECDSA using curve secp256k1; supports signing and
  signature verification. Intended for blockchain signing; behaves differently
  from the NIST curves in several ways — see
  [secp256k1 keys](#secp256k1-keys) below
...
The `ecdsa-secp256k1` key type exists for blockchain signing, where the curve
is used by Bitcoin, Ethereum, and systems built on them. It deliberately
behaves differently from the NIST curves in ways you need to know about.

**It signs a digest you supply, without rehashing it.** Unlike
`ecdsa-p256` and friends, this key type does not hash its input. The `input`
parameter to `transit/sign` must be the base64 encoding of exactly 32 bytes,
and any other length is rejected rather than hashed. You do not need
`prehashed=true`. Recognized `hash_algorithm` values do not change the digest;
unknown algorithm names are still rejected by the API.

This is intentional. Blockchain signatures are made over a digest produced by a
domain-separating scheme that OpenBao cannot reconstruct — an Ethereum RLP
transaction hash, an EIP-191 `personal_sign` digest, or an EIP-712 struct hash —
and all of them use Keccak-256, which is not one of Transit's hash algorithms.
Note that Transit's `sha3-256` is FIPS-202 SHA-3 and is **not** the same
function as Ethereum's Keccak-256; they use different padding. Compute the
digest in your client and send it.
...
signatures use the low-S form required by Ethereum transaction validation and
commonly used contract verification libraries. Verification accepts either form.
The EVM `ecrecover` precompile itself accepts high-S signatures, while libraries
such as OpenZeppelin ECDSA reject them; Bitcoin requirements also depend on the
applicable script and relay policy. A `valid: true` response establishes the
ECDSA signature's validity, not compliance with a particular verifier's rules.

**Recovering the Ethereum `v` byte is the client's job.** Transit returns
ASN.1 DER (`marshaling_algorithm=asn1`, the default) or a raw 64-byte `r||s`
(`marshaling_algorithm=jws`). Neither carries the 1-byte recovery id that
`ecrecover` needs. Clients derive it by trying each candidate against the known
public key, exactly as they already must for AWS KMS and Google Cloud KMS.
Note also that `marshaling_algorithm=jws` only selects the raw `r||s` byte
layout — Transit does not emit a JOSE header, so the RFC 8812 `ES256K`
algorithm identifier is the client's concern.

**Auto-rotation is refused.** A verifier identifies this key by the blockchain
address derived from its public key, and that address is what gets enrolled on
chain. Rotating the key changes the address, and clients commonly cache the
address for their process lifetime, so a silent rotation would leave them
signing under an address the chain no longer accepts. A nonzero
`auto_rotate_period` is therefore rejected both at creation and via
`keys/config`. Manual rotation is
still possible, but treat it as a deliberate operation paired with enrolling
the new address. For the same reason, be careful with `min_encryption_version`
on a rotated key: raising it past a version can make key material you still
need unusable for signing.

**Not supported for this key type:** key import (BYOK), BYOK export, key agreement
(`derive-key`/ECDH), CSR generation and certificate binding, and encryption. Go's
standard library does not recognise the secp256k1 curve OID
...
This private fork supports secp256k1 key creation through the CLI and HTTP API.
The UI's key-creation dropdown does not offer this key type.
